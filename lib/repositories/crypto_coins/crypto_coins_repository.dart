...
class CryptoCoinsRepository implements AbstractCoinsRepository {
  ...
  Future<List<CryptoCoin>> getCoinsList() async {
    ...
    try {
      cryptoCoinsList = await _fetchCryptoCoinsList();
  ...
  Future<List<CryptoCoin>> _fetchCryptoCoinsList() async {
    final response = await dio.get(
        'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BTC,NEIROCOIN,GRASS,SOL&tsyms=USD');
  ...
  Future<CryptoCoin> _fetchCoinDetailsFromApi(String currencyCode) async {
    ...
    final data = response.data as Map<String, dynamic>;
    final dataRaw = data['RAW'] as Map<String, dynamic>;
    final coinDetail = dataRaw[currencyCode] as Map<String, dynamic>;
    final usdData = coinDetail['USD'] as Map<String, dynamic>;
    final details = CryptoCoinDetail.fromJson(usdData);
